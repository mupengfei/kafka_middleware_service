...
RUN mkdir /opt/java &&\
	wget --no-check-certificate --no-cookies --header "Cookie: oraclelicense=accept-securebackup-cookie" http://download.oracle.com/otn-pub/java/jdk/8u111-b14/jdk-8u111-linux-x64.tar.gz -P /opt/java

RUN mkdir /opt/kafka &&\
	wget http://apache.fayea.com/kafka/$KAFKA_VERSION/kafka_2.11-$KAFKA_VERSION.tgz -P /opt/kafka

RUN tar zxvf /opt/java/jdk-8u111-linux-x64.tar.gz -C /opt/java &&\
...
RUN echo "source /root/.bash_profile" > /opt/kafka/start.sh &&\
	...
	echo "[ ! -z $""HOST_NAME"" ] && echo "host.name='$HOST_NAME'" >> /opt/kafka/kafka_2.11-"$KAFKA_VERSION"/config/server.properties" >> /opt/kafka/start.sh &&\
	echo "[ ! -z $""ADVERTISED_HOST_NAME"" ] && echo "advertised.listeners='PLAINTEXT://$ADVERTISED_HOST_NAME:$BROKER_PORT'" >> /opt/kafka/kafka_2.11-"$KAFKA_VERSION"/config/server.properties" >> /opt/kafka/start.sh &&\
	echo "[ ! -z $""ADVERTISED_HOST_NAME"" ] && echo "listeners='PLAINTEXT://$ADVERTISED_HOST_NAME:$BROKER_PORT'" >> /opt/kafka/kafka_2.11-"$KAFKA_VERSION"/config/server.properties" >> /opt/kafka/start.sh &&\
